...
class MitmProxy(HTTPServer):
    ...
    def __init__(self, server_address=('', 8080), RequestHandlerClass=ProxyHandler, bind_and_activate=True, ca_file='ca.pem'):
        server_address = GetIPDataFromOpenshiftEnvironment()
...
def GetIPDataFromOpenshiftEnvironment():
    try:
        listenIP = environ['OPENSHIFT_PYTHON_IP']
    except KeyError:
        listenIP = ""

    try:
        listenPort = int(environ['OPENSHIFT_PYTHON_PORT'])
    except KeyError:
        listenPort = 8080
